...
export class MouseTooltip extends HudObject
{
    constructor(name)
    {
        ...
        this.leftText.graphic.scale.y = -1;
        ...
        this.rightText.graphic.scale.y = -1;
    ...
    update()
    {
        if(MOUSE_OVER_OBJECT != null)
        {
            if(MOUSE_OVER_OBJECT instanceof InterfaceSkillSlot)
            {
                ...
                this.rightText.setPosition(this.leftText.getScreenBounds().x + this.leftText.getScreenBounds().width, this.rightText.graphic.position.y);
